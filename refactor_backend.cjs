...
function getMutationCaseBody(content, caseName) {
  ...
  // Strip outer braces
  let inner = block.trim().substring(1, block.length - 1).trim();
...
Object.entries(helpers).forEach(([name, content]) => {
  fs.writeFileSync(path.join(SRC, 'context', 'helpers', name + '.ts'), content.trim() + '\n', 'utf8');
});
...
const providerOpenBrace = findFunctionBodyBrace(navigationContent, navigationContent.indexOf('AppNavigationProvider'));
const providerBracedBlock = getBracedBlock(navigationContent, providerOpenBrace);
const navigationContextTypeBlock = extractBlock(navigationContent, 'NavigationContextType', 'interface');
...
const TEMPORARY_RECEIPT_SCAN_ERROR_MATCH = serverFnsContent.match(/const TEMPORARY_RECEIPT_SCAN_ERROR\s*=\s*[^;]+;/);
const DEFAULT_GEMINI_RECEIPT_SCAN_FALLBACK_MODELS_MATCH = serverFnsContent.match(/const DEFAULT_GEMINI_RECEIPT_SCAN_FALLBACK_MODELS\s*=\s*\[[^\]]+\];/);
...
Object.entries(mutations).forEach(([name, content]) => {
  fs.writeFileSync(path.join(SRC, 'server', 'mutations', name + '.ts'), content.trim() + '\n', 'utf8');
});
...
const mutationsIndexContent = Object.keys(mutations)
  ...
  .join('\n');
fs.writeFileSync(path.join(SRC, 'server', 'mutations', 'index.ts'), mutationsIndexContent + '\n', 'utf8');
...
translationsSplit.en = '// For English, we return the key itself\nexport const en: Record<string, string> = {};\n';
...
Object.entries(translationsSplit).forEach(([name, content]) => {
  fs.writeFileSync(path.join(SRC, 'lib', 'translations', name + '.ts'), content.trim() + '\n', 'utf8');
});
