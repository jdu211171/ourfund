...
export function AppNavigationProvider({ children }: { children: ReactNode }) {
  ...
  const contributeToGoal = (
    ...
    const selectedWallet = walletId ? activeWallets.find(w => w.id === walletId) : activeWallets[0]
  ...
  const updateScheduleItem = (id: string, updates: Partial<Omit<ScheduleItem, 'id'>>) => {
    const current =
      recurringIncome.find(item => item.id === id) ?? subscriptions.find(item => item.id === id)
    if (!current) return
  ...
  const updateLoanEntry = (id: string, updates: Partial<Omit<LoanEntry, 'id' | 'createdAt'>>) => {
    setLoanEntries(prev => prev.map(entry => (entry.id === id ? { ...entry, ...updates } : entry)))
