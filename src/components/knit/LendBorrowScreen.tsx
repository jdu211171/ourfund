...
import { type LoanDirection, type LoanEntry, useAppNavigation } from '@/lib/navigation'
...
export function LendBorrowScreen() {
  ...
  const savePayment = () => {
    ...
    const updatedStatus =
      remainingUsd <= 0.01 ? 'paid' : paymentLoan.status === 'paid' ? 'pending' : paymentLoan.status
