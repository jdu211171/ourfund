...
export function AddIncomeScreen() {
  ...
  const [source, setSource] = useState('Monthly salary')

  const defaultSources = ['Monthly salary', 'Freelance invoice', 'Allowance', 'Gift']
  const sources = [...new Set([...defaultSources, source])]
  ...
  const incomePresets = useMemo(() => {
    ...
    const incomes = transactions.filter(t => t.usd > 0)
    let recentIncomes = incomes.filter(t => t.date >= oneMonthAgoStr)

    if (recentIncomes.length < 5) {
      recentIncomes = incomes
    }

    const counts = new Map<string, { count: number; lastTx: (typeof recentIncomes)[0] }>()
    for (const tx of recentIncomes) {
      const key = `${tx.name.trim().toLowerCase()}||${tx.wallet.trim().toLowerCase()}`
  ...
  const handleApplyPreset = (preset: (typeof incomePresets)[0]) => {
    ...
    <PhoneFrame>
      <div className="flex-1 overflow-y-auto flex flex-col px-7 pt-10 pb-7 min-h-0">
        <header className="flex items-center justify-between">
          <button
            onClick={goBack}
            className="grid h-9 w-9 place-items-center rounded-full text-foreground hover:bg-slate-50 transition-colors border-none bg-transparent"
            aria-label="Back"
          >
        ...
        <div className="mt-6 grid flex-1 grid-cols-3 place-items-center gap-y-1">
          {keys.map(k => (
            <button
              ...
              className="grid h-12 w-12 place-items-center text-[26px] font-semibold text-foreground transition-colors active:bg-[var(--muted)] rounded-full hover:bg-slate-50 cursor-pointer border-none bg-transparent"
        ...
        <button
          ...
          className="mt-4 w-full rounded-full bg-[var(--primary)] py-4 text-[15px] font-semibold text-white active:scale-95 transition-transform cursor-pointer border-none"
        >
