...
export const SYNONYM_GROUPS: SynonymGroup[] = [
  ...
  },
  {
    ...
    category: 'Bakery',
    tokens: ['パン', '食パン', 'ブレッド', 'ローフ', 'トースト', '山型', '角型', 'bread', 'loaf']
  },
  {
    canonical: 'Tomatoes',
    category: 'Produce',
    tokens: ['トマト', 'とまと', 'ミニトマト', 'tomato', 'tomatoes']
  },
  {
    canonical: 'Bananas',
    category: 'Produce',
    tokens: ['バナナ', 'ばなな', 'banana', 'bananas']
  },
  {
    canonical: 'Apples',
    category: 'Produce',
    tokens: ['りんご', 'リンゴ', '林檎', 'ふじ', 'サンふじ', 'つがる', 'apple', 'apples']
  },
  {
    canonical: 'Chicken breast 1kg',
    category: 'Meat',
    tokens: ['鶏肉', 'とり肉', '鶏むね', 'むね肉', 'ブロイラー', '若鶏', 'chicken', 'breast']
  },
  {
    canonical: 'Tofu',
    category: 'Meat', // protein group
    tokens: ['豆腐', 'とうふ', '絹豆腐', '木綿豆腐', '絹ごし', '木綿', 'tofu']
  },
  {
    canonical: 'Yogurt 400g',
    category: 'Dairy',
    tokens: ['ヨーグルト', 'よーぐると', 'プレーンヨーグルト', 'yogurt', 'yoghurt']
  },
  {
    canonical: 'Olive oil 1L',
    category: 'Pantry',
    tokens: ['オリーブオイル', 'オリーブ油', 'エクストラバージン', 'olive oil', 'oliveoil']
  },
  {
    canonical: 'Coffee beans 200g',
    category: 'Pantry',
    tokens: ['コーヒー', '珈琲', 'コーヒー豆', 'アラビカ', 'coffee', 'beans']
  },
  {
    canonical: 'Pasta 500g',
    category: 'Pantry',
    tokens: ['パスタ', 'スパゲッティ', 'スパゲティ', 'マカロニ', 'pasta', 'spaghetti']
  }
]
...
export function resolveCanonical(name: string): string {
  ...
  const sortedTokens = Array.from(TOKEN_TO_CANONICAL.keys()).sort((a, b) => b.length - a.length)
...
export function categorize(name: string): Category {
  ...
  if (exact) return exact
  for (const token of Array.from(TOKEN_TO_CATEGORY.keys()).sort((a, b) => b.length - a.length)) {
  ...
  if (/banana|apple|tomato|onion|lettuce|veg|fruit|potato|carrot/.test(norm)) return 'Produce'
  if (/chicken|beef|pork|fish|meat|tofu|egg/.test(norm)) return 'Meat'
  if (/bread|loaf|bun|bagel|pastry/.test(norm)) return 'Bakery'
  if (/rice|pasta|oil|coffee|tea|sugar|salt|flour|sauce/.test(norm)) return 'Pantry'
  return 'Other'
}
...
export function findCheapest(name: string, dbProducts: any[] = []): PurchaseRecord | null {
...
export function recentPrice(name: string, dbProducts: any[] = []): PurchaseRecord | null {
  if (!name.trim()) return null
  const allHits = [
    ...dbProducts.filter(p => matches(name, p.name)).map(dbToRecord),
    ...PURCHASE_HISTORY.filter(r => matches(name, r.name))
...
export function priceHistoryFor(name: string, dbProducts: any[] = []): PurchaseRecord[] {
  if (!name.trim()) return []
  const allHits = [
    ...dbProducts.filter(p => matches(name, p.name)).map(dbToRecord),
    ...PURCHASE_HISTORY.filter(r => matches(name, r.name))
